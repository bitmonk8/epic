...
// session as `epic <verb>` custom commands. The `NuShell` tool provides direct
...
pub enum AgentMethod {
    ...
    Analyze,
    /// Leaf execution — needs write + nu.
    Execute,
    /// Design and decompose — needs nu for exploration.
    Decompose,
}

/// Returns the tool grant set appropriate for a given agent method.
///
/// All tools route through the nu session, so NU is always included.
/// WRITE is only granted for Execute (leaf execution).
pub fn phase_tools(method: AgentMethod) -> ToolGrant {
    match method {
        AgentMethod::Execute => ToolGrant::WRITE | ToolGrant::NU,
        AgentMethod::Analyze | AgentMethod::Decompose => ToolGrant::NU,
    }
}
...
///
/// All file tools execute as nu custom commands (`epic read`, etc.).
...
/// Grep) are translated into `epic <verb>` nu commands. `NuShell` is a direct
...
mod tests {
    ...
    #[test]
    fn phase_tool_matrix() {
        for method in [AgentMethod::Analyze, AgentMethod::Decompose, AgentMethod::Execute] {
            let grant = phase_tools(method);
            let tools = tool_definitions(grant);
            let names: Vec<&str> = tools.iter().map(|t| t.name.as_str()).collect();
            assert!(names.contains(&"Read"), "{method:?} missing Read");
            assert!(names.contains(&"NuShell"), "{method:?} missing NuShell");
            if matches!(method, AgentMethod::Execute) {
                assert!(names.contains(&"Write"), "Execute missing Write");
                assert!(names.contains(&"Edit"), "Execute missing Edit");
            } else {
                assert!(!names.contains(&"Write"), "{method:?} has Write");
                assert!(!names.contains(&"Edit"), "{method:?} has Edit");
            }
        }
    }
