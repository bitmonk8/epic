...
fn build_context_populates_parent_fields_and_children() {
    ...
    let ctx = orch.build_context(&state, child_id).unwrap();
    assert_eq!(
    ...
    let parent_ctx = orch.build_context(&state, parent_id).unwrap();
...
fn build_context_child_status_mapping_all_phases() {
    ...
    let ctx = orch.build_context(&state, parent_id).unwrap();
