...
1. **BUG: AppContainer NUL device access (lot API ready, epic integration needed)** — `epic grep` (rg) fails inside AppContainer because AppContainer blocks `\\.\NUL` device access. lot now provides `nul_device_accessible()`, `can_modify_nul_device()`, `grant_nul_device_access()`. Remaining: add `epic setup` CLI subcommand + startup check in `run`/`resume`. See `docs/WINDOWS_SANDBOX.md`.
