...
- 6 tools: `read_file`, `glob`, `grep`, `write_file`, `edit_file`, `bash` — path sandboxing, size limits, timeout handling
...
- Process sandboxing via lot — bash tool spawns children inside OS-native sandbox (AppContainer on Windows, namespaces+seccomp on Linux, Seatbelt on macOS); sandbox is mandatory (no unsandboxed fallback)
